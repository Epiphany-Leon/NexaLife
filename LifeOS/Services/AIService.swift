...
private enum ConnectionInsightEngine {
	static func fallback(
		...
		switch importance {
		case 5:
			attitude = "高信任 + 高频沟通，优先维护"
		case 4:
			attitude = "稳定投入，围绕互惠价值保持联系"
		case 3:
			attitude = "保持温和连接，按周期跟进"
		default:
			attitude = "轻量维护，避免过度投入"
		}
		...
		if let date = lastContactDate {
			let days = Calendar.current.dateComponents([.day], from: date, to: Date()).day ?? 0
			nextAction = days > 30 ? "在 48 小时内发送一次近况问候并提出一个具体可帮忙点。" : "维持当前沟通节奏，下一次沟通聚焦双方共同目标。"
		} else {
			nextAction = "先建立最近联系记录，再决定跟进频率。建议先发一条简短近况问候。"
		}
...
class AIService: ObservableObject {
	...
	func classifyText(_ text: String) async -> AppModule {
		guard isConfigured else {
			return TaskSuggestionEngine.moduleSuggestion(for: text)
		}
		let prompt = """
		你是 LifeOS 的智能助手。根据以下文本，判断最适合归入哪个象限。
		...
		"""
		guard let response = await callAPI(prompt: prompt, maxTokens: 20) else { return .inbox }
		return AppModule(rawValue: response.trimmingCharacters(in: .whitespacesAndNewlines)) ?? .inbox
	}
	...
	func analyzeConnection(
		...
		let dateText = lastContactDate?.formatted(date: .abbreviated, time: .omitted) ?? "未记录"
		let prompt = """
	...
	func generateReport(entries: [String], type: String) async -> String {
		guard isConfigured else {
			return "⚠️ 请先在「偏好设置 → AI 设置」中配置 API Key"
		}
		let prompt = """
		基于以下「\(type)」记录，生成一份简洁的个人总结报告（中文，300字以内，分段落）：
		\(entries.joined(separator: "\n---\n"))
		"""
		return await callAPI(prompt: prompt, maxTokens: 500) ?? "报告生成失败，请检查网络与 API Key。"
	}
